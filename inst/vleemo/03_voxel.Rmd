...
Voor de eenvoud definiëren we de voxel als een kubus met zijde `r params$voxel_size`.
...
Om rekening te houden met de windrichting kozen we om de vogeltracks te roteren rond de positie van de windmodel zodat alle geroteerde tracks noordenwind hebben.
Een track in noordwest richting bij westenwind zal na de rotatie een track in noordoost richting bij noordenwind zijn.
...
De windmolen in de figuur zijn alle windmolens beschikbaar in OpenStreetMap in de omgeving van de Antwerpse haven.
...
Elke deelfiguur toont een horizontale snede op de verschillende hoogtes.
...
```{r voxel-raw, fig.cap = "Overzicht van het totaal aantal tracks per voxel over alle geselecteerde windmodels. In oranje geven we de positie van de mast en rotor weer in 2 dimensies weer. De deelfiguren geven de verschillende hoogtes weer.", fig.height = 7}
voxel |>
...
De activiteit $Y_{ixyz}$ in voxel $xy$ in laag $z$ rond windmodel $i$ volgens een Poisson verdeling met gemiddelde $\mu_{ixyz}$ \@ref(eq:Y).
...
We veronderstellen dat het windmolen effect uit een Gaussianse verdeling met gemiddelde $0$ en standaard afwijking $\sigma_w$ \@ref(eq:windmill).
Het effect van de hoogte modelleren we als een tweede orde toevalsbeweging (second order random walk) \@ref(eq:hoogte).
Het effect van de voxel $\xi(xy,z)$ modelleren we als horizontale lagen die een Gaussian Markov Random Field (GMRF) volgen waarbij twee boven elkaar gelegen lagen een autogressive model van order 1 volgen \@ref(eq:field).
...
\end{equation}
```
In figuur \@ref(fig:voxel-fitted-windmill) geven we het effect de windmolen.
Het verschil tussen de windmolen met hoogste en laagste activiteit is begrenst omdat we enkel de windmolens gebruiken die minstens 10% van de windmolen met hoogste activiteit hebben.
...
Onder de 100 m stellen we een sterke afnamen van de activiteit vast.
Dit is mogelijk te wijten aan beperkingen qua zichtbaarheid door de radar.

```{r voxel-fitted-height, fig.cap = "Gemiddelde activiteit in functie van de hoogte. Relatief ten opzichte van de globaal gemiddelde activiteit."}
delta_label <- function(x) {
  sprintf("%.1f%%", 100 - 100 * exp(x))
}
...
Tussen 90 en 210 m stellen we een sterk patroon van in de horizontale lagen.
Vlakbij de windmolen is de activiteit beduidend lager dan rond de windmolen.
Merk op dat dit bereik min of meer overeenkomt met het rotorvlak.
...
We generen een set van aselecte punten in de omgeving van windmolen zodat de voxelrasters van de controleset dichtbij de windmolen zijn, zonder overlap tussen de voxelrasters (fig. \@ref(fig:voxel-alt-windmolen)).
...
De controleset bestaat uit `r sum(alt_centre$type == "controle")` punten, terwijl de originele set uit `r sum(gebruikt$gebruikt)` punten bestaat.
...
gebruikt |>
  mutate(type = ifelse(.data$gebruikt, "windmolen", "windmolen\nmet lage activiteit")) |>
...
Wanneer we naar de ruwe gegevens van de controleset kijken stellen we opnieuw een sterk effect van de hoogte vast (fig. \@ref(fig:voxel-raw-alt)).
Om een betere vergelijking te kunnen maken, fitten we hetzelfde statistisch model aan de controleset.
...
Bij de controleset stellen we minder sterke correlaties vast (tabel \@ref(tab:voxel-model-hyperpar-alt)).
De correlatie tussen de horizontal lagen daalt van $0.90$ naar $0.79$.
...
Enkel de variabiliteit van het hoogteeffect verdubbelt nagenoeg.
Het hoogteeffect beschrijven we met een tweede orde toevalsbeweging.
...
Waar we rond de windmolens op lage hoogte sterke patronen vast stellen (fig. \@ref(fig:voxel-fitted-field)), zijn deze bij de controlepunten slechts in beperkte mate aanwezig (fig. \@ref(fig:voxel-fitted-field-alt)).
De kleurgradient in beide figuren gebruikt dezelfde kleurschaal zodat we ze gemakkelijk met elkaar kunnen vergelijken.
...
Kijken we enkel naar de globale verschillen dan blijkt de activiteit rond windmodels gemiddelde `r verschil` te bedragen van de activiteit rond de nabijgelegen controlepunten.

Boven 300 m stellen we geen significant verschil in relevatieve activiteit vast (fig. \@ref(fig:voxel-fitted-height-control)).
...
In figuur \@ref(fig:voxel-fitted-field-controle2-effect) hebben de de verschillen opgedeeld in een aantal klassen.
...
```{r voxel-fitted-field-controle2, fig.cap = "Relatief verschil in gemiddelde activiteit rond windmolen versus de controle.", fig.height = 7.5}
field_control2 |>
...
In figuur \@ref(fig:voxel-fitted-field-controle) geven we het verschil wanneer rekening houden met alle effecten, behalve dat van de individuele windmolen.
We combineren met andere woorden het globaal gemiddelde, het hoogteeffect en het ruimtelijke patroon.
...
```{r voxel-fitted-field-controle-effect, fig.cap = "Classificatie relatief verschil in gemiddelde activiteit rond windmolen versus de controle.", fig.height = 7.5}
field_control |>
  ggplot(aes(x = x, y = y, fill = effect)) +
  geom_raster() +
  scale_effect(labels = ef_labels) +
